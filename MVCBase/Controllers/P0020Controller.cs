    ...
    public class P0020Controller : Controller
    {
            ...
            switch (btnSubmit)
            {
                case "新增":
                    ...
                    if (!ModelState.IsValid) 
                    {
                        ...
                        ViewBag.ErrorList = GetModelStateErrorList(); // 示範如何顯示所有的錯誤.
        ...
        public ActionResult SubmitUpdate(FormCollection form1)  // 參數中不放任何model或變數, 就可以解除ModelBinder自動處理.
        {
            ...
            switch (btnSubmit)
            {
                ...
                case "刪除":
                    ...
                    t1.ms1 = form1["ms1"]; // 若從ModelBinder機制取得的話, 則會觸發其他不必要的欄位檢核邏輯.
                    if (string.IsNullOrEmpty(t1.ms1))
                    ...
                    {
                        P0020 oP0020 = new P0020();
                        if (!oP0020.Delete(t1.ms1)) // 商業邏輯
                            if (string.IsNullOrEmpty(oP0020.msError))
                                ModelState.AddModelError(oP0020.ToString(), "刪除失敗, 請聯絡系統管理人員取得失敗原因.");
                            else
                                ModelState.AddModelError(oP0020.ToString(), oP0020.msError);
                    }
                    // 若有錯誤時, 則留在原畫面並顯示錯誤資訊.
                    if (!ModelState.IsValid)
                    {
                        ...
                        P0010ViewModel vm1 = new P0020().ConvertModelToViewModel(t1.Read1Record(t1.ms1));
        ...
        public string TestSubmitModelBinder(TestModelBinder e, string ms5, string btnSubmit)
        {
            ...
            return "ms1=" + e.ms1 + // 比對property名稱符合
                ...
                + GetModelStateErrorList();
        }

        public string GetModelStateErrorList()
        {
            // 示範取得ModelState清單
            ...
            {
                if (!ModelState.IsValidField(m1.Key))
                {
